...
function extractTitlea() {
  const title1 = document.querySelectorAll(
    'div[class = "u-fontSize24 u-xs-fontSize18"]'
  );
...
function extractUsername() {
  const username = document.querySelectorAll(
    'div[class = "postMetaInline postMetaInline-authorLockup ui-captionStrong u-flex1 u-noWrapWithEllipsis"] > a'
  );
  const items = [];
...
function extractDate() {
  const date = document.querySelectorAll(
    'div[class = "ui-caption u-fontSize12 u-baseColor--textNormal u-textColorNormal js-postMetaInlineSupplemental"] > time'
  );
  const items = [];
...
function extractBodya() {
  const body1 = document.querySelectorAll(
    'div[class = "u-fontSize18 u-xs-fontSize16"]'
  );
